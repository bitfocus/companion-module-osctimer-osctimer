...
function getActionDefinitions(self) {
	return {
		...
		'timer_start': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/start')
			}
		},
		'timer_stop': {
			name: 'Stop Timer',
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/stop')
			}
		},
		'timer_reset': {
			name: 'Reset Timer',
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/reset')
			}
		},
		'timer_reset_and_stop': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/resetAndStop')
			}
		},
		...
		'timer_set_seconds': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/count/seconds', [event.options.seconds])
			}
		},
		'timer_set_minutes': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/count/minutes', [event.options.minutes])
			}
		},
		'timer_set_hours': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/count/hours', [event.options.hours])
			}
		},
		'timer_set_time': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/count/time', [
					event.options.hours, 
		...
		'timer_add_seconds': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/count/seconds/add', [event.options.seconds])
			}
		},
		'timer_subtract_seconds': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/count/seconds/subtract', [event.options.seconds])
			}
		},
		...
		'timer_count_direction': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/count/direction', [event.options.direction])
			}
		},
		...
		'timer_show_format': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/show', [event.options.format])
			}
		},
		...
		'timer_set_alert_seconds': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/alert/seconds', [event.options.seconds])
			}
		},
		...
		'set_notes_alignment': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/notes/alignment', [event.options.alignment])
			}
		},
		...
		'set_timer_end_behavior': {
			...
			callback: (event) => {
				self.sendCommand(event.options.timerNum, '/atEnd', [event.options.behavior])
			}
		}
	}
}
